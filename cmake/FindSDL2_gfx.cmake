...
if(NOT SDL2_GFX_INCLUDE_DIR AND SDL2GFX_INCLUDE_DIR)
    set(SDL2_GFX_INCLUDE_DIR ${SDL2GFX_INCLUDE_DIR} CACHE PATH "directory cache entry initialized from old variable name")
endif()
find_path(SDL2_GFX_INCLUDE_DIR SDL_gfx.h
        HINTS
        ENV SDL2GFXDIR
        ENV SDL2DIR
        ${SDL2_DIR}
...
if(NOT SDL2_GFX_LIBRARY AND SDL2GFX_LIBRARY)
    set(SDL2_GFX_LIBRARY ${SDL2GFX_LIBRARY} CACHE FILEPATH "file cache entry initialized from old variable name")
endif()
...
if(SDL2_GFX_INCLUDE_DIR AND EXISTS "${SDL2_GFX_INCLUDE_DIR}/SDL2_gfx.h")
    file(STRINGS "${SDL2_GFX_INCLUDE_DIR}/SDL2_gfx.h" SDL2_GFX_VERSION_MAJOR_LINE REGEX "^#define[ \t]+SDL2_GFX_MAJOR_VERSION[ \t]+[0-9]+$")
    file(STRINGS "${SDL2_GFX_INCLUDE_DIR}/SDL2_gfx.h" SDL2_GFX_VERSION_MINOR_LINE REGEX "^#define[ \t]+SDL2_GFX_MINOR_VERSION[ \t]+[0-9]+$")
    file(STRINGS "${SDL2_GFX_INCLUDE_DIR}/SDL2_gfx.h" SDL2_GFX_VERSION_PATCH_LINE REGEX "^#define[ \t]+SDL2_GFX_PATCHLEVEL[ \t]+[0-9]+$")
    string(REGEX REPLACE "^#define[ \t]+SDL2_GFX_MAJOR_VERSION[ \t]+([0-9]+)$" "\\1" SDL2_GFX_VERSION_MAJOR "${SDL2_GFX_VERSION_MAJOR_LINE}")
    string(REGEX REPLACE "^#define[ \t]+SDL2_GFX_MINOR_VERSION[ \t]+([0-9]+)$" "\\1" SDL2_GFX_VERSION_MINOR "${SDL2_GFX_VERSION_MINOR_LINE}")
    string(REGEX REPLACE "^#define[ \t]+SDL2_GFX_PATCHLEVEL[ \t]+([0-9]+)$" "\\1" SDL2_GFX_VERSION_PATCH "${SDL2_GFX_VERSION_PATCH_LINE}")
